...
public partial class FirmaDigital : System.Web.UI.Page
{
    ...
    private void CargarCertificados()
    {
        ...
        LblCertInfo.Text = count == 0
            ? "No se encontraron certificados con clave privada. Inserte el DNIe y haga clic en Refrescar."
            : string.Format("{0} certificado(s) disponible(s). Verifique que el DNIe este insertado.", count);
    }
                    ...
                    private IEnumerable<X509Certificate2> EnumerarCertificadosDisponibles()
                    {
                        ...
                        foreach (var item in stores)
                        {
                             foreach (var cert in EnumerarCertificadosDeStore(new X509Store(item.Item1, item.Item2)))
                             {
                                 ...
                                 if (EsProviderBit4id(cert)) continue;
                    ...
                    protected void BtnFirmarDni_Click(object sender, EventArgs e)
                    {
                        ...
                        try
                        {
                            ...
                            if (selectedCert == null)
                                throw new Exception("No se pudo obtener el certificado seleccionado. Verifique que el DNIe este conectado y haga clic en Refrescar.");
                    ...
                    private void RegistrarFirma(byte[] contenidoFirmado, X509Certificate2 selectedCert)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(connStr))
                        {
                            using (SqlCommand cmd = new SqlCommand("FIR_X_RegistrarFirma_OUT", conn))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@CertificadoSN", selectedCert.SerialNumber ?? "DNIe");
                                ...
                                cmd.Parameters.AddWithValue("@RespuestaMotor", "Firma DNIe Windows");
                    ...
                    private float FitFontSize(Graphics g, string text, float maxWidth, float maxHeight, float min, float max)
                    {
                     ...
                     private string NormalizarThumbprint(string value)
                     {
                      ...
                      private bool EsProviderBit4id(X509Certificate2 cert)
                      {
                          if (cert == null) return false;

                          // No excluir certificados de RENIEC o ECEP
                          string emisor = cert.GetNameInfo(X509NameType.SimpleName, true);
                          ...
                          // Obtener el provider name desde propiedades del contexto del certificado (sin hardware check)
                          string providerName = ObtenerNombreProviderPropiedad(cert);
                          if (!string.IsNullOrEmpty(providerName))
                          {
                              return providerName.IndexOf("Bit4id", StringComparison.OrdinalIgnoreCase) >= 0;
                          }
                          return false;
                      }
...
public class LegacySmartCardSignature : iTextSharp.text.pdf.security.IExternalSignature
{
    ...
    private static string ObtenerContenedorPropiedad(X509Certificate2 cert)
    {
        if (cert == null || cert.Handle == IntPtr.Zero) return null;
        uint pcbData = 0;
        const uint CERT_KEY_PROV_INFO_PROP_ID = 2;
        ...
        try
        {
            ...
            {
                var provInfo = (CRYPT_KEY_PROV_INFO)System.Runtime.InteropServices.Marshal.PtrToStructure(pvData, typeof(CRYPT_KEY_PROV_INFO));
                return provInfo.pwszContainerName;
            }
        }
        catch {}
        finally
    ...
    {
        // 1. Intentar forzar CNG de forma explícita usando el KSP de Smart Card
        // Esto evita el wrapper de CAPI a CNG que falla con el error 'tarjeta no compatible'
        string containerName = ObtenerContenedorPropiedad(_cert);
        if (!string.IsNullOrEmpty(containerName))
        {
            try
            {
                var provider = new System.Security.Cryptography.CngProvider("Microsoft Smart Card Key Storage Provider");
                using (System.Security.Cryptography.CngKey cngKey = System.Security.Cryptography.CngKey.Open(containerName, provider))
                using (System.Security.Cryptography.RSACng rsaCng = new System.Security.Cryptography.RSACng(cngKey))
                {
                    string netHashName = _hashAlgorithm.Replace("-", "");
                    return rsaCng.SignData(message, new System.Security.Cryptography.HashAlgorithmName(netHashName), System.Security.Cryptography.RSASignaturePadding.Pkcs1);
                }
            }
            catch (Exception ex)
            {
                // Si el usuario canceló la firma (PIN incorrecto, botón cancelar, etc.), propagamos la excepción
                if (ex.Message.Contains("cancelada") || ex.Message.Contains("canceled") || ex.Message.Contains("cancelled"))
                {
                    throw;
                }
                // Para otros errores, intentamos los fallbacks
            }
        }

        // Fallback 1: GetRSAPrivateKey estándar de .NET
        try
        {
